...
function extractSupabaseUrlFromDatabaseUrl(dbUrl: string): string | null {
  if (!dbUrl) return null;
  
  // Example username pattern: postgres.wbyytzsbhacmzizlvqhz
  const usernameMatch = dbUrl.match(/postgres\.([a-zA-Z0-9_-]+):/);
  if (usernameMatch && usernameMatch[1]) {
    return `https://${usernameMatch[1]}.supabase.co`;
  }
  
  // Example host pattern: db.projectid.supabase.co
  const hostMatch = dbUrl.match(/@db\.([a-zA-Z0-9_-]+)\.supabase\.(co|com)/);
  if (hostMatch && hostMatch[1]) {
    return `https://${hostMatch[1]}.supabase.co`;
  }
  
  const genMatch = dbUrl.match(/@([a-zA-Z0-9_-]+)\.supabase\.(co|com)/);
  if (genMatch && genMatch[1] && genMatch[1] !== 'pooler') {
    return `https://${genMatch[1]}.supabase.co`;
  }

  try {
    const parts = dbUrl.split('@');
    if (parts.length > 1) {
      const credentialsSegment = parts[0];
      const indexOfCred = credentialsSegment.indexOf('://');
      const userPass = indexOfCred !== -1 ? credentialsSegment.substring(indexOfCred + 3) : credentialsSegment;
      const user = userPass.split(':')[0];
      if (user.startsWith('postgres.')) {
        const projectId = user.substring(9);
...
function resolveSupabaseConfig() {
  ...
  let key = (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_KEY || '').trim();
  ...
  console.log('--- resolveSupabaseConfig starting diagnostic:', { 
    hasUrl: !!url, 
  ...
  const looksLikeKey = (s: string) => s.startsWith('sb_') || s.startsWith('eyJ') || s.length > 30 || (!isUrl(s) && s.length > 5);

  // 1. Swap check: if SUPABASE_URL looks like a key and the key looks like a URL, swap them!
  if (looksLikeKey(url) && isUrl(key)) {
    console.log('--- resolveSupabaseConfig: Detected swapped URL and Key! Swapping them...');
    const temp = url;
    url = key;
    key = temp;
  }

  // 2. Derive URL check: if the URL is not valid or actually looks like a key, attempt extraction
  if (!isUrl(url)) {
    console.log('--- resolveSupabaseConfig: SUPABASE_URL is not a valid URL. Extracting from DATABASE_URL...');
    const derivedUrl = extractSupabaseUrlFromDatabaseUrl(dbUrl);
    if (derivedUrl) {
      console.log('--- resolveSupabaseConfig: Successfully derived url:', derivedUrl);
      if (looksLikeKey(url) && !key) {
        console.log('--- resolveSupabaseConfig: Reusing the key found in SUPABASE_URL:', url.substring(0, 10) + '...');
        key = url;
      }
      url = derivedUrl;
    }
...
function getSupabase() {
  if (!supabaseClient) {
    ...
    console.log('--- getSupabase final resolution:', {
      resolvedUrl: url,
      hasKey: !!key,
    ...
    if (!url || !key) {
      throw new Error(`SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be configured to upload images. Resolved URL: "${url || 'missing'}", Resolved Key: ${key ? 'present' : 'missing'}`);
    }

    if (!url.startsWith('http://') && !url.startsWith('https://')) {
      throw new Error(`SUPABASE_URL must be a valid HTTP or HTTPS URL. Resolved: "${url}"`);
    }
...
async function uploadBase64Image(base64Data: string, sku: string): Promise<string> {
  // If not a data URI (it is already a standard URL), return as-is
  ...
  // Ensure bucket exists (try, but don't fail if it exists or listBuckets is restricted)
  try {
    ...
    console.warn('Silent bucket check/creation warning:', err);
  }

  const cleanSku = (sku || 'item').replace(/[^a-zA-Z0-9_-]/g, '');
...
// --- Phase 4: RBAC Enforcement ---
  ...
  app.get('/api/debug-supabase', (req, res) => {
    ...
    const mask = (val: string) => {
      if (!val) return 'empty/falsy';
      const trimmed = val.trim();
      if (trimmed.length <= 8) return `${'*'.repeat(trimmed.length)} (length: ${trimmed.length})`;
      return `${trimmed.substring(0, 4)}...${trimmed.substring(trimmed.length - 4)} (length: ${trimmed.length})`;
    };

    res.json({
      SUPABASE_URL: {
        raw: mask(supabaseUrl),
        isValidUrl: supabaseUrl.trim().startsWith('http://') || supabaseUrl.trim().startsWith('https://'),
        trimmedAndClean: supabaseUrl.trim().replaceAll('"', '').replaceAll("'", "")
      },
      SUPABASE_SERVICE_ROLE_KEY: {
        raw: mask(serviceRoleKey)
      },
      SUPABASE_KEY: {
        raw: mask(supabaseKey)
      },
  ...
  app.post('/api/inventory', checkPermission('Inventory'), async (req: any, res) => {
    try {
      const { name, sku, category, stock, minStock, price, status, description, display_stock, warehouse_stock, brand, product_line, size, packing, unit, color, image } = req.body;
      ...
      if (image && image.startsWith('data:image/')) {
        try {
          finalImageUrl = await uploadBase64Image(image, sku);
        } catch (uploadError: any) {
          console.error('Failed to upload image to Supabase:', uploadError);
          return res.status(400).json({ error: 'Supabase Storage: ' + uploadError.message });
        }
      }
  ...
  app.put('/api/inventory/:id', checkPermission('Inventory'), async (req: any, res) => {
    try {
      ...
      if (product_line !== undefined) updateData.product_line = product_line;
      if (size !== undefined) updateData.size = size;
      if (packing !== undefined) updateData.packing = packing;
      ...
      if (image !== undefined) {
        if (image && image.startsWith('data:image/')) {
          try {
            updateData.image = await uploadBase64Image(image, sku || oldState.sku);
          } catch (uploadError: any) {
            console.error('Failed to upload image to Supabase:', uploadError);
            return res.status(400).json({ error: 'Supabase Storage: ' + uploadError.message });
          }
  ...
  app.post('/api/purchases', checkPermission('Purchases'), async (req: any, res) => {
    ...
    try {
      ...
      for (const item of items) {
        ...
        // Update warehouse stock - default to Main Factory for purchases
        const factoryWh = await trx('warehouse_stocks')
          .where({ inventory_id: item.product_id, warehouse_name: 'Main Factory (A-1)' })
          .first();

        if (factoryWh) {
          await trx('warehouse_stocks').where({ id: factoryWh.id }).increment('stock', item.qty);
        } else {
          await trx('warehouse_stocks').insert({
        ...
        // Sync display_stock / warehouse_stock columns in inventory table
        const whStocks = await trx('warehouse_stocks').where({ inventory_id: item.product_id });
        let wTotal = 0;
        let dTotal = 0;
        whStocks.forEach((s: any) => {
          if (s.warehouse_name === 'Shop Display') dTotal += s.stock;
          else wTotal += s.stock;
        });
        
        await trx('inventory').where({ id: item.product_id }).update({
  ...
  app.get('/api/factory/dashboard', checkPermission('Production'), async (req, res) => {
    try {
      ...
      const todayBatches = await db('production_batches')
        ...
        .select('production_batches.*', 'inventory.category as product_category');
      ...
      for (const batch of recentBatches) {
        ...
        if (h >= 6 && h < 10) slots[0].value += qty;
        else if (h >= 10 && h < 14) slots[1].value += qty;
        else if (h >= 14 && h < 18) slots[2].value += qty;
        else if (h >= 18 && h < 22) slots[3].value += qty;
        else if (h >= 22 || h < 2) slots[4].value += qty;
        else if (h >= 2 && h < 6) slots[5].value += qty;
      }
      ...
      const result = await db.transaction(async (trx) => {
        ...
        if (materials && Array.isArray(materials)) {
          for (const m of materials) {
            const qtyConsumed = Number(m.qty_consumed || 0);
            ...
            if (!matWh || Number(matWh.stock || 0) < qtyConsumed) {
            ...
            await trx('warehouse_stocks').where({ id: matWh.id }).decrement('stock', qtyConsumed);
  ...
  app.get('/api/analytics/dashboard', checkPermission('Dashboard'), async (req, res) => {
    try {
      ...
      // 2. Weekly Production Efficiency (Real calculation)
      const efficiencyStats = await db('production_batches')
        .where('status', 'Completed')
        .where('completed_at', '>=', new Date(Date.now() - 7 * 86400000).toISOString())
        .select(
      ...
      const productionEfficiency = efficiencyStats && efficiencyStats.total_target > 0 
        ? Math.round((efficiencyStats.total_actual / efficiencyStats.total_target) * 100)
        : 0;
  ...
  // Simple in-memory cache for slow aggregates
  const reportsCache = new Map<string, { data: any, expiresAt: number }>();
  ...
  const setCachedValue = (key: string, data: any, ttlMs = 5000) => {
    reportsCache.set(key, { data, expiresAt: Date.now() + ttlMs });
  };

  // Reports & Analytics API
  app.get('/api/reports/sales', checkPermission('Reports'), async (req, res) => {
    const { period = 'daily' } = req.query;
    const cacheKey = `reports_sales_${period}`;
    ...
    try {
      ...
      setCachedValue(cacheKey, results, 5000);
      res.json(results);
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Failed' });
  ...
  app.get('/api/reports/city-sales', checkPermission('Reports'), async (req, res) => {
    const cacheKey = 'reports_city_sales';
    const cached = getCachedValue(cacheKey);
    if (cached) return res.json(cached);
  ...
  app.get('/api/reports/outstanding', checkPermission('Reports'), async (req, res) => {
    const cacheKey = 'reports_outstanding';
    const cached = getCachedValue(cacheKey);
    if (cached) return res.json(cached);
    ...
    try {
      ...
      setCachedValue(cacheKey, formatted, 5000);
      res.json(formatted);
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Failed' });
  ...
  app.get('/api/reports/inventory-valuation', checkPermission('Reports'), async (req, res) => {
    const cacheKey = 'reports_inventory_valuation';
    const cached = getCachedValue(cacheKey);
    if (cached) return res.json(cached);
    ...
    try {
      ...
      const results = { items: valuation, total: totalValuation };
  ...
  app.get('/api/reports/production-summary', checkPermission('Reports'), async (req, res) => {
    const cacheKey = 'reports_production_summary';
    const cached = getCachedValue(cacheKey);
    if (cached) return res.json(cached);
    ...
    try {
      ...
      setCachedValue(cacheKey, summary, 5000);
      res.json(summary);
    } catch (error) {
      res.status(500).json({ error: 'Failed' });
    }
  ...
  // --- LABOUR API ---
  app.get('/api/labour', checkPermission('Labour'), async (req, res) => {
    try {
      const labour = await db('labour').select('*').orderBy('id', 'desc');
      res.json(labour);
    } catch (error) {
      console.error(error);
  ...
  app.post('/api/labour', checkPermission('Labour'), async (req, res) => {
    try {
      const { name, cnic, address, mobile_no, designation, salary } = req.body;
      const insertResult = await db('labour').insert({ name, cnic, address, mobile_no, designation, salary }).returning('id');
      const id = typeof insertResult[0] === 'object' ? insertResult[0].id : insertResult[0];
      const labour = await db('labour').where({ id }).first();
      await auditLog(req, 'CREATE', 'Labour', { id, name });
  ...
  app.put('/api/labour/:id', checkPermission('Labour'), async (req, res) => {
    try {
      const { name, cnic, address, mobile_no, designation, salary } = req.body;
      await db('labour').where({ id: req.params.id }).update({ name, cnic, address, mobile_no, designation, salary, updated_at: db.fn.now() });
      const labour = await db('labour').where({ id: req.params.id }).first();
      await auditLog(req, 'UPDATE', 'Labour', { id: req.params.id, name });
      res.json(labour);
    } catch (error) {
      console.error(error);
  ...
  app.delete('/api/labour/:id', checkPermission('Labour'), async (req, res) => {
    try {
      const labour = await db('labour').where({ id: req.params.id }).first();
      await db('labour').where({ id: req.params.id }).delete();
      await auditLog(req, 'DELETE', 'Labour', { id: req.params.id, name: labour?.name });
      res.json({ success: true });
    } catch (error) {
      console.error(error);
