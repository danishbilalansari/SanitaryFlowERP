...
export default function Accounts() {
  ...
  const totalCredits = React.useMemo(() => {
    return ledger.reduce((acc, curr) => acc + (curr.credit || 0), 0);
  }, [ledger]);

  const totalDebits = React.useMemo(() => {
    return ledger.reduce((acc, curr) => acc + (curr.debit || 0), 0);
  }, [ledger]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="bg-white border border-[#edeeef] p-6 rounded-2xl shadow-sm">
            ...
            <h3 className="text-[28px] font-black text-[#162839]">{formatCurrency(totalCredits, currency)}</h3>
         </div>
         <div className="bg-white border border-[#edeeef] p-6 rounded-2xl shadow-sm">
            <div className="flex justify-between items-start mb-4">
            ...
            <h3 className="text-[28px] font-black text-[#162839]">{formatCurrency(totalDebits, currency)}</h3>
         </div>
         <div className="bg-[#162839] p-6 rounded-2xl shadow-xl">
            ...
            <h3 className="text-[28px] font-black text-white">{formatCurrency(balance, currency)}</h3>
         </div>
      </div>
               ...
               {filteredLedger.map((row) => (
                <tr key={row.id} className="hover:bg-[#f8f9fa] transition-colors group">
                  ...
                  <td className="px-8 py-5 text-right">
                    {row.debit > 0 && <span className="font-black text-red-500">-{formatCurrency(row.debit, currency)}</span>}
                  </td>
                  <td className="px-8 py-5 text-right">
                    {row.credit > 0 && <span className="font-black text-emerald-500">+{formatCurrency(row.credit, currency)}</span>}
                  </td>
                </tr>
              ))}
