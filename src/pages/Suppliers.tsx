      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-neutral-200 p-6 rounded-xl shadow-sm">
           ...
           <h3 className="text-[28px] font-bold text-red-500">
             {currency}{suppliers.reduce((acc, s) => acc + (s.balance > 0 ? s.balance : 0), 0).toLocaleString()}
           </h3>
        </div>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:flex-none">
            <select 
              ...
              className="w-full md:w-[180px] justify-center items-center gap-2 px-4 py-3 bg-white border border-[#edeeef] rounded-lg text-[13px] font-bold text-neutral-600 hover:bg-neutral-50 transition-colors whitespace-nowrap flex appearance-none pr-10"
            >
                ...
                <tr key={s.id} className="hover:bg-[#f8f9fa] transition-colors group">
                  ...
                  <td className="px-8 py-5 text-right font-black text-[#162839]">
                    <span className={s.balance > 0 ? "text-red-500" : "text-emerald-600"}>
                      {currency}{Math.abs(s.balance).toLocaleString()}
                    </span>
                  </td>
        ...
        {totalPages > 1 && (
          <div className="px-8 py-4 bg-[#f8f9fa] border-t border-[#edeeef] flex justify-between items-center">
            <span className="text-[13px] font-bold text-neutral-400">
              Showing <span className="text-[#162839]">{(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredSuppliers.length)}</span> of <span className="text-[#162839]">{filteredSuppliers.length}</span> suppliers
            </span>
