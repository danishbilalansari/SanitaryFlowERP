              ...
              {showDropdown && searchQuery && (
                <div className="absolute top-full left-0 right-0 mt-1 max-h-60 overflow-y-auto border bg-white shadow-lg rounded-xl divide-y z-20">
                  {filteredItems.length > 0 ? filteredItems.map(item => (
                    <div key={item.id} onClick={() => handleAddItem(item)} className="p-3 hover:bg-blue-50 cursor-pointer flex justify-between gap-4">
                      <div className="flex gap-3 min-w-0">
                        <div className="w-10 h-10 rounded bg-neutral-100 flex items-center justify-center shrink-0">
                          <ItemImage src={item.image} name={item.name} className="w-full h-full object-cover rounded text-[10px]" />
                        </div>
                        <div className="min-w-0">
                        <p className="font-bold">{item.name}</p>
                        <p className="text-xs text-neutral-500 truncate">{item.description || item.product_line || 'No description'}</p>
                        <p className="text-xs text-neutral-400">{item.sku} • Size: {item.size || '-'} • Color: {item.color || '-'} • Stock: {item.stock ?? 0}</p>
                        </div>
                      </div>
                      <span className="font-bold shrink-0">{formatCurrency(item.price ?? 0, currency)}</span>
                    </div>
                         ...
                         <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-neutral-400">{item.sku} • Size: {item.size || '-'} • Color: {item.color || '-'} • Stock: {item.max ?? 0}</p>
                              </div>
                            </div>
                         </td>
        ...
        <section className="col-span-12 lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#162839] p-8 rounded-2xl shadow-xl text-white">
             ...
             <button 
                ...
                disabled={saving}
                className="w-full bg-[#5cb8fd] text-[#00476e] font-black py-4 rounded-xl hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
             >
                {saving && (
                   <div className="w-5 h-5 border-2 border-[#00476e]/30 border-t-[#00476e] rounded-full animate-spin" />
                )}
