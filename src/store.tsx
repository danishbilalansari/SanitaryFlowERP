...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [companyName, setCompanyName] = useState<string>(() => {
    ...
    if (saved === 'SanitaryFlow Solutions Ltd.' || saved === 'SanitaryFlow Solutions Ltd' || saved === 'SanitaryFlow SOlutions Ltd') {
      return 'SanitaryFlow';
    }
    return saved || 'SanitaryFlow';
  });
  const [companyAddress, setCompanyAddress] = useState<string>(() => localStorage.getItem('company_address') || '12 Industrial Park, Sector 4\nAdministrative Block, HQ-500021');
  const [companyEmail, setCompanyEmail] = useState<string>(() => localStorage.getItem('company_email') || 'admin@sanitaryflow.com');
  const [companyPhone, setCompanyPhone] = useState<string>(() => localStorage.getItem('company_phone') || '+1 (555) 092-3481');
